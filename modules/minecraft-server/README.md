...
Every Minecraft server you've ever joined is a program running on somebody's
computer. This module puts that program on yours — and then teaches you programming,
networking, and how computers really work by changing everything about it.

The server in this module is not a practice exercise. It's real. Friends and family
can play on it, and everything you learn shows up in front of them:

- You'll add crafting recipes that don't exist in Minecraft — and watch your friends
  discover them.
- You'll open your server to the internet, safely, and someone across town will walk
  around a world running on your machine.
...
- You'll speak the Minecraft network protocol yourself, one byte at a time, and use
  it to put a live "who's online" page on every phone in your house.
- When something goes wrong — and you will make things go wrong on purpose — you'll
  restore a destroyed world from a backup system you built yourself.
...
The order below runs from "set up a machine" to "your code is infrastructure," with
each payoff funding the next. You don't have to follow it — every lesson states its own
requirements, and if you meet them you can do that lesson whenever you like. The order
is here for anyone who'd rather be handed a sensible sequence than work one out.
...
1. **[Setting up a coding machine](../dev-machine/lessons/dev-machine-setup/guided.md)** —
   ...
   version control. This lesson lives in the
   [Setting up a dev machine](../dev-machine/README.md) module because it isn't
   Minecraft-specific, but it's step one here all the same.
2. **[Running your own server](lessons/running-your-own-server/guided.md)** — a
   Minecraft server you can start, stop, join, and break without consequences.
   Everything else in this module touches this server one way or another.

### The server is yours to change

3. **[Server settings and console commands](lessons/server-settings-and-console/guided.md)**
   — read every line of the server's settings file and start overruling it, then
   drive the running server live from its console.
4. **[Building datapacks](lessons/building-datapacks/guided.md)** — custom crafting
   recipes and server-side functions, written in plain text files: the first things
   you invent that other players use.

### Letting people in

5. **[Letting friends join your server](lessons/letting-friends-join/guided.md)** —
   from someone on your wifi to friends across the internet, safely: addresses,
   ports, the three routes out of the house, and the whitelist. The module's
   central milestone lives here.

### Protecting what people build

6. **[Copying and backing up worlds](lessons/worlds-and-backups/guided.md)** —
   worlds become files you can copy; a backup script you wrote and proved by
   destroying a world and bringing it back; backups without kicking anyone off.
7. **[Keeping the server always on](lessons/always-on/guided.md)** — survives
...
9. **[Running your server on a rented machine](../remote-server/README.md)** — a module
   of its own, five lessons long: rent a Linux machine, lock it down, move your world
...
Different lessons need different things, and each one states its own list. Across
the whole module, the recurring ingredients are: a Mac you're allowed to install
software on, Minecraft: Java Edition with an account that can play it, a home wifi
network, and — for the best parts — friends or family who play. Nothing here
requires paying for hosting; your own machine is the server.
