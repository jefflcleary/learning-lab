...
- **A computer set up for coding** — a terminal you can move around in, and a
  `projects` folder. [Setting up a coding machine](../dev-machine-setup/guided.md) gets you
...
install it to run the server, and nothing in this course will ask you to write it.
...
you've met the concept that the entire letting-people-in part of this course is
...
- You proved one machine can't run two servers *on one port*. So can one machine run
  two servers? What would have to be different between them? Try it if you dare —
  everything in this folder can be rebuilt.
...
- The habit this whole course leans on: when a program refuses, the reason is in
  what it printed
